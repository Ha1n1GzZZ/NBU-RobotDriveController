...
int64_t trans_direction_to_pulses(int32_t distance) {
    int64_t pulsenum = 0;  
    pulsenum =  abs(distance)*3200/(2*3.14*Pulley_radius);                   //此处用于计算移动距离与脉冲之间的关系
    return pulsenum;
}
...
void StepperMotor_SetPosition(int32_t positon) { //传入需要到达的位置,单位mm
    ...
    int64_t pulsenum = trans_direction_to_pulses(distance);
    if(distance*direction>0)
    {	
        for(int i=0;i<pulsenum;i++)
        {
            Reversal_Stepper;
            delay_us(250);   //该步进电机最大空载启动频率为1500pps,最小延时时间为666us,但是实操目前尚未发现问题
